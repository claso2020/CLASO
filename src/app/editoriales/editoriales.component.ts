...
export class EditorialesComponent implements OnInit {
  displayedColumns: string[] = ['acciones','nombre','habilitada'];
  dataSource = new MatTableDataSource();
  escondeTabla: boolean = true;
  
  @ViewChild(MatPaginator, {static: true}) paginador: MatPaginator;
  @ViewChild(MatSort, {static: true}) ordenar: MatSort;
  @ViewChild('TablaEditoriales',null) tablaEditoriales: ElementRef;
  ...
  modificarEditorial(idEditorial) {
    this.storage.set("idGeneral",idEditorial);
    this.router.navigate(['/editoriales-mod'])
  }
  ...
  exportarAExcel()
    {
      ...
      var myTimer = setInterval(() => {
        const ws: XLSX.WorkSheet=XLSX.utils.table_to_sheet(this.tablaEditoriales.nativeElement);//converts a DOM TABLE element to a worksheet
        this.delete_cols(ws,0,0);
        const wb: XLSX.WorkBook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Hoja1');
  
        /* save to file */
        XLSX.writeFile(wb, 'editoriales.xlsx');
